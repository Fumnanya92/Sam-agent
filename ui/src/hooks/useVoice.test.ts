...
describe("matchesSpeechWakePhrase (strict; used during TTS playback)", () => {
  test("accepts direct wake phrases", () => {
    expect(matchesSpeechWakePhrase("Sam")).toBe(true);
    expect(matchesSpeechWakePhrase("hey sam")).toBe(true);
    expect(matchesSpeechWakePhrase("Sam, stop")).toBe(true);
    expect(matchesSpeechWakePhrase("Hey Sam, hold on")).toBe(true);
  });

  test("rejects longer sentences that merely mention sam (echo protection)", () => {
    expect(matchesSpeechWakePhrase("Sam is already working on that")).toBe(false);
    expect(matchesSpeechWakePhrase("Can you tell Sam to send that")).toBe(false);
    expect(matchesSpeechWakePhrase("I said Sam in the middle of a sentence")).toBe(false);
    expect(matchesSpeechWakePhrase("Hey Sam can you help")).toBe(false);
  });
});

describe("matchesSpeechWakePrefix (loose; used when idle)", () => {
  test("accepts bare wake phrases", () => {
    expect(matchesSpeechWakePrefix("Sam")).toBe(true);
    expect(matchesSpeechWakePrefix("hey sam")).toBe(true);
  });

  test("accepts wake phrase followed by any command in one utterance", () => {
    expect(matchesSpeechWakePrefix("hey sam turn off the lights")).toBe(true);
    expect(matchesSpeechWakePrefix("sam what's the weather")).toBe(true);
    expect(matchesSpeechWakePrefix("Hey Sam, play some music")).toBe(true);
    expect(matchesSpeechWakePrefix("sam can you help me")).toBe(true);
  });

  test("rejects utterances that merely contain sam mid-sentence", () => {
    expect(matchesSpeechWakePrefix("Can you tell Sam to send that")).toBe(false);
    expect(matchesSpeechWakePrefix("I said Sam in the middle of a sentence")).toBe(false);
    expect(matchesSpeechWakePrefix("that was Sam talking")).toBe(false);
  });
...
describe("matcher normalization (shared by both matchers)", () => {
  ...
  test("collapses stacked punctuation and multiple spaces", () => {
    expect(matchesSpeechWakePhrase("Sam... stop!!")).toBe(true);
    expect(matchesSpeechWakePrefix("hey   sam    play   music")).toBe(true);
  });

  test("is not fooled by mid-word 'sam' substrings", () => {
    // "starsam" is one token; without a word boundary it must not match.
    expect(matchesSpeechWakePrefix("starsam")).toBe(false);
    expect(matchesSpeechWakePhrase("starsam stop")).toBe(false);
  });
});
