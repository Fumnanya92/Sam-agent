...
function getWakePrefix(normalized: string): "hey sam " | "sam " | null {
  if (normalized.startsWith("hey sam ")) return "hey sam ";
  if (normalized.startsWith("sam ")) return "sam ";
  return null;
}

/**
 * Strict matcher used during TTS playback (voiceState === "speaking").
 * Accepts bare wake phrases ("sam", "hey sam") or wake + a short
 * whitelisted interrupt command. Prevents Sam's own TTS from self-triggering
 * when the reply contains the word "sam" inside a sentence.
 */
export function matchesSpeechWakePhrase(transcript: string): boolean {
  const normalized = normalizeTranscript(transcript);
  if (!normalized) return false;
  if (normalized === "sam" || normalized === "hey sam") return true;
 ...
 * wake phrase, so natural "hey sam turn off the lights" wakes in one breath
 * without waiting for Chrome to emit a bare-wake interim first.
 * NOT safe to use while Sam is speaking — use matchesSpeechWakePhrase there.
 */
...
export type UseVoiceReturn = {
  ...
  micLevel: number; // 0–100 during recording, 0 otherwise
  cancelTTS: () => void;
...
export function useVoice({ wsRef, wakeWordEnabled = true, wakeEngine = "openwakeword" }: UseVoiceOptions): UseVoiceReturn {
  ...
  const startWakeWordEngine = useCallback(async () => {
    ...
    try {
      ...
      console.log("[Voice] Wake word engine ready — say 'Hey Sam'");
    } catch (err) {
  ...
  const startSpeechWakeIfNeeded = useCallback((): void => {
    ...
    if (!speechWakeRef.current) {
      ...
      recognition.onresult = (event: SpeechRecognitionEvent) => {
        ...
        // loose prefix matcher when idle so "hey sam <command>" wakes in one breath.
    ...
    try {
      ...
      console.log("[Voice] Speech wake recognizer starting — say 'Sam' or 'Hey Sam'");
    } catch {
    ...
    try {
      ...
      if (autoStop) {
        ...
        silenceCheckRef.current = setInterval(() => {
          ...
          // Update mic level for visual feedback (0–100)
          setMicLevel(Math.min(100, Math.round((avg / 25) * 100)));
