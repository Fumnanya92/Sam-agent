...
export type TutorialStepPayload = {
  ...
  instruction: string;
  imageBase64?: string;
  highlight?: { x: number; y: number; width: number; height: number; label?: string };
  complete?: boolean;
};
...
export type ChatMessage = {
  ...
  source?: string; // 'heartbeat', 'proactive', 'sub-agent', 'screen_view', 'tutorial_step', 'test_result'
  priority?: string;
  isStreaming?: boolean;
  screenView?: ScreenViewPayload;
  tutorialStep?: TutorialStepPayload;
...
function createSidecarNotice(payload: SidecarEventPayload, timestamp?: number): ChatMessage & { notice?: SystemNotice } {
  ...
  const notice: SystemNotice = {
    ...
    text: reason
      ? `Sam daemon disconnected: ${reason}. Dashboard features may be delayed until it reconnects.`
      : "Sam daemon disconnected. Dashboard features may be delayed until it reconnects.",
    level: "warning",
  };
...
export function useWebSocket() {
  ...
  const [takeoverState, setTakeoverState] = useState<TakeoverState | null>(null);
  ...
  const handleMessage = useCallback((msg: WSMessage) => {
    if (msg.type === "chat" && msg.payload?.source) {
    ...
    } else if (msg.type === "screen_view") {
      const sv = msg.payload as ScreenViewPayload;
      if (!sv.active) {
        // Panel closed — freeze the existing panel in-place
        setMessages((prev) =>
          prev.map((m) =>
            m.source === "screen_view" && m.screenView?.viewId === sv.viewId
              ? { ...m, screenView: { ...m.screenView!, active: false } }
              : m
          )
        );
        return;
      }
      // Update existing panel or create a new one
      setMessages((prev) => {
        const idx = prev.findIndex((m) => m.source === "screen_view" && m.screenView?.viewId === sv.viewId);
        if (idx !== -1) {
          const updated = [...prev];
          updated[idx] = { ...updated[idx]!, screenView: sv };
          return updated;
        }
        return [
          ...prev,
          {
            id: `screen-view-${sv.viewId}`,
            role: "system",
            content: "",
            timestamp: msg.timestamp,
            source: "screen_view",
    ...
    } else if (msg.type === "takeover_event") {
      const te = msg.payload as { state: string; task?: string; stepNarration?: string };
      if (te.state === "active") {
        setTakeoverState((prev) => ({
          active: true,
          task: te.task ?? prev?.task,
          stepNarration: te.stepNarration,
        }));
      } else {
        setTakeoverState(null);
      }
    } else if (msg.type === "tutorial_step") {
      const step = msg.payload as TutorialStepPayload;
      setMessages((prev) => [
        ...prev,
        {
          id: `tutorial-${step.tutorialId}-${step.stepIndex}`,
          role: "system",
      ...
      const tr = msg.payload as TestResultPayload;
      setMessages((prev) => {
        // Update existing test result card in-place if same testRunId + testName
        const idx = prev.findIndex(
          (m) => m.source === "test_result" && m.testResult?.testRunId === tr.testRunId && m.testResult?.testName === tr.testName
        );
        if (idx !== -1) {
          const updated = [...prev];
          updated[idx] = { ...updated[idx]!, testResult: tr };
          return updated;
        }
        return [
          ...prev,
          {
            id: `test-${tr.testRunId}-${tr.testName.replace(/\s+/g, "-")}`,
            role: "system",
            content: tr.testName,
            timestamp: msg.timestamp,
            source: "test_result",
  ...
  const cancelTakeover = useCallback(() => {
    ...
    wsRef.current.send(JSON.stringify({
      type: "command",
      payload: { command: "cancel_takeover" },
      timestamp: Date.now(),
    }));
    setTakeoverState(null);
  }, []);
  ...
  return {
    ...
    takeoverState, cancelTakeover, notices, dismissNotice,
    wsRef,
    voiceCallbacksRef,
  };
}
