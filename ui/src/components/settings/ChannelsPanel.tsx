...
export function ChannelsPanel() {
  ...
  const saveChannels = async () => {
    try {
      ...
      setTgToken("");
      setDcToken("");
      setMsg({ text: "Channel config saved. Restart Sam to apply.", type: "success" });
      refetchCfg();
      refetchStatus();
    } catch (err) {
  ...
  const saveSTT = async () => {
    try {
      ...
      setSttKey("");
      setMsg({ text: "STT config saved. Restart Sam to apply.", type: "success" });
      refetchStt();
    } catch (err) {
      setMsg({ text: err instanceof Error ? err.message : "Failed to save", type: "error" });
    }
  ...
  return (
    <div style={cardStyle}>
      ...
      <div style={{ ...sectionStyle, marginTop: "16px", borderBottom: "none" }}>
        ...
        <p style={hintStyle}>
          Enables voice responses from Sam via the dashboard.
        </p>
