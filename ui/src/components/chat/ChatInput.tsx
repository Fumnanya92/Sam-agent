...
export function ChatInput({ onSend, disabled, voice }: Props) {
  ...
  const getMicTitle = () => {
    ...
    if (voice.isWakeWordReady) return `Say "Hey Sam" or click to speak${engineSuffix}`;
  ...
  return (
    <div className="chat-input-area">
      <div className="chat-input-row">
        ...
        {/* Mic button with level visualizer */}
        {voice?.isMicAvailable && (
          <div className="chat-mic-wrap">
            {voice.voiceState === "recording" && (
              <div className="chat-mic-level">
                {[0.4, 0.7, 1.0, 0.7, 0.4].map((scale, i) => {
                  const height = Math.max(4, Math.round((voice.micLevel / 100) * 22 * scale));
                  return (
                    <div
                      key={i}
                      className="chat-mic-level-bar"
