...
| Understand failures | `agent/error_handler.py`, `agent/executor.py`, `actions/code_helper.py` | `sam_v2/supervisor/recovery.py`, diagnostics modules | Migrated But Unverified | Feed Sam a real failing build/test run and require a useful explanation and next action | run tests/builds, logging | High | 23 |
