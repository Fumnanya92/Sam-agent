# actions/desktop.py
# AI-powered desktop & wallpaper management — adapted from Mark-XXX-main.
...
def _ask_ai_for_desktop_action(task: str) -> str:
    ...
    system_prompt = (
        "You are a Windows desktop automation expert. "
        "Generate safe Python code using ONLY these allowed modules: "
        "pyautogui (mouse/keyboard/screenshots), pathlib.Path, "
        "shutil (ONLY: copy2, copytree, move, disk_usage), "
        "os.path (ONLY: exists, join, dirname, basename, splitext), "
        "time (sleep only), ctypes (Windows API calls only), "
        "winreg (registry READ only). "
        "Rules: Output ONLY the Python code. No explanation, no markdown, no backticks. "
        "NO file deletion, NO subprocess calls, NO exec() or eval(), NO file writes. "
        "If task cannot be done safely, output exactly: UNSAFE"
    )
    user_prompt = f"Desktop path: {desktop}\n\nTask: {task}\n\nPython code:"
...
def _execute_generated_code(code: str) -> str:
    """Safely executes AI-generated desktop automation code."""
...
def organize_desktop(mode: str = "by_type") -> str:
    """Organizes desktop files by type or date."""
...
def clean_desktop() -> str:
    """Moves all files on desktop into a timestamped archive folder."""
    desktop     = _get_desktop()
...
def desktop_control(
    ...
    try:
        if action == "wallpaper":
            ...
            print(f"[Desktop] Asking AI: {actual_task}")
            if player:
            ...
            if code == "UNSAFE":
                result = "Cannot perform that desktop action safely."
            ...
            if full_task:
                code   = _ask_ai_for_desktop_action(full_task)
