...
# YouTube Controller — adapted from Mark-XXX-main.
# Gemini calls replaced with agent/llm_bridge.py (Ollama-first, OpenAI fallback).
#
# Features:
#   - play      : Search and play a YouTube video
#   - summarize : Get transcript → AI summary → speak + optional save
...
def find_video_thumbnails() -> list:
    try:
...
def _ask_for_url(prompt_text: str = "YouTube video URL:"):
    try:
        ...
        url = simpledialog.askstring("Sam", prompt_text, parent=root)
...
def _summarize_with_ai(transcript: str) -> str:
...
def _handle_play(parameters: dict, player) -> str:
    ...
    if len(thumbnails) >= 2:
        ...
        print("[YouTube] No thumbnails found, using fallback position")
...
def _handle_summarize(parameters: dict, player, speak) -> str:
    ...
    if not transcript:
        return (
            "I couldn't retrieve a transcript for that video. "
    ...
    try:
        summary = _summarize_with_ai(transcript)
    except Exception as e:
    ...
    if save:
        ...
        header = f"Sam — YouTube Summary\n{'─'*50}\nURL    : {url}\n{'─'*50}\n\n"
        saved_path = save_to_desktop_text(header + summary, "youtube_summary")
...
def youtube_video(
    ...
    Actions:
        ...
        summarize : Summarize a video via transcript + AI
                    parameters: save(bool, optional) — save summary to Desktop
        ...
        trending  : Show trending videos
                    parameters: region(str, default "US") — ISO country code
    """
