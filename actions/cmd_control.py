...
# Auto-executes natural language system commands (safe, no approval required).
# Different from terminal.py which requires human approval for each command.
# Gemini replaced with Sam's llm_bridge.
...
_CMD_SYSTEM = (
    "Convert the user's request to a single Windows CMD command.\n"
    "Output ONLY the command. No explanation, no markdown, no backticks.\n"
    "If unsafe or impossible, output: UNSAFE"
)


def _ask_ai(task: str) -> str:
    try:
        ...
        command = result.strip().strip("`").strip()
...
def cmd_control(
    ...
    if not command:
        ...
        if command:
            ...
            print(f"[CMD] AI fallback for: {task}")
            command = _ask_ai(task)
