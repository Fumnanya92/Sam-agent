...
_BROWSER_BINARIES = {
    "Windows": {
        ...
        "vivaldi": ["vivaldi.exe"], "chrome": ["chrome.exe"], "firefox": ["firefox.exe"],
    },
    "Darwin": {
        "opera": ["opera"], "brave": ["brave browser", "brave"],
        "vivaldi": ["vivaldi"], "chrome": ["google chrome", "google-chrome"], "firefox": ["firefox"],
    },
    "Linux": {
...
def _find_browser_executable(prog_id: str) -> tuple:
    ...
    browser_patterns = {
        "brave": ["brave"], "vivaldi": ["vivaldi"], "chrome": ["chrome"],
    }
...
class _BrowserThread:
    ...
    async def _scan_form_fields(self) -> list[dict]:
        """Return all visible input fields from the accessibility tree.

        Each entry: {"name": str, "role": str}
        This lets Sam see exactly what the form contains before guessing.
        """
        page = await self._get_page()
        try:
        ...
        def _walk(node):
            ...
            if role in ("textbox", "searchbox", "combobox", "spinbutton") and name:
    ...
    async def _fill_form_auto(self, fields: dict) -> str:
        """Accessibility-tree-first form fill.

        1. Scans the page accessibility tree for all input fields.
        2. Matches each requested field description to the closest actual input name.
        3. Fills using get_by_role(name=...) — the most reliable Playwright locator.

        fields: {user_description: value_to_type}
        e.g. {"Full name": "Jane Doe", "Email address": "jane@example.com"}
        """
        page = await self._get_page()
        known_fields = await self._scan_form_fields()
        results = []
        ...
        for description, value in fields.items():
            ...
            # 1. Try exact accessible-name match first (most reliable)
            for field in known_fields:
                if desc_lower == field["name"].lower():
                    try:
                        loc = page.get_by_role(field["role"], name=field["name"])
                        await loc.first.clear()
                        await loc.first.type(str(value), delay=40)
                        results.append(f"OK (exact) {description[:30]}")
                        filled = True
                        break
                    except Exception:
                        pass
            ...
            # 2. Try substring accessible-name match
            for field in known_fields:
                field_name_lower = field["name"].lower()
                if desc_lower in field_name_lower or field_name_lower in desc_lower:
                    try:
                        loc = page.get_by_role(field["role"], name=field["name"])
                        await loc.first.clear()
                        await loc.first.type(str(value), delay=40)
                        results.append(f"OK (match) {description[:30]}")
                        filled = True
                        break
                    except Exception:
                        pass
            ...
            # 3. Fallback: smart_type heuristic (placeholder / label / first textbox)
            res = await self._smart_type(description, str(value))
            ok = "Could not" not in res and "error" not in res.lower()
            results.append(f"{'OK (heuristic)' if ok else 'FAIL'} {description[:30]}")

        return "Form filled: " + ", ".join(results)

    async def _fill_form_smart(self, fields: dict) -> str:
        """Fill form fields by description (placeholder / label / role) rather than CSS.

        fields: {description_or_label: value_to_type}
        e.g. {"Full name": "Jane Doe", "Email address": "jane@example.com"}
        """
        results = []
        for description, value in fields.items():
            res = await self._smart_type(description, str(value))
            ok = "Could not" not in res and "error" not in res.lower()
            results.append(f"{'OK' if ok else 'FAIL'} {description[:30]}")
        return "Form filled: " + ", ".join(results)
...
def browser_control(
    ...
    """
    Playwright browser automation — auto-detects system default browser.

    parameters:
        action      : go_to | search | click | type | scroll |
                      fill_form | fill_form_smart | fill_form_auto | scan_form |
        ...
        fields      : {label/description: value} for fill_form_auto/fill_form_smart
                      OR {CSS_selector: value} for fill_form (legacy)
    ...
    try:
        ...
        elif action == "fill_form_smart":
            result = _bt.run(_bt._fill_form_smart(parameters.get("fields", {})))
        elif action == "fill_form_auto":
            result = _bt.run(_bt._fill_form_auto(parameters.get("fields", {})))
        elif action == "scan_form":
            fields = _bt.run(_bt._scan_form_fields())
            result = "Form fields found: " + ", ".join(
                f['name'] for f in fields
            ) if fields else "No form fields detected on this page."
