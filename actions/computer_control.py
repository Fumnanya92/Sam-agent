...
# Atomic computer control via PyAutoGUI — click, type, drag, screenshot, screen_find.
# screen_find vision call adapted from Gemini to OpenAI via Sam's llm_bridge.
...
def _load_user_profile() -> dict:
    memory_path = _BASE_DIR / "memory" / "memory.json"
    try:
...
_SCREEN_FIND_SYSTEM = (
    "You are analyzing a screenshot to locate a UI element. "
    "The user will describe an element. Return ONLY: x,y "
    "(the center pixel coordinates of the element). "
...
def _analyze_screen_for_element(description: str) -> tuple[int, int] | None:
    """
    Takes a screenshot and asks OpenAI vision to find x,y of described element.
    Replaces Mark's Gemini vision call.
    """
    try:
        import re as _re
        ...
        prompt    = (
            f"Screenshot resolution: {w}x{h} pixels.\n"
            f"Find this element: '{description}'\n"
            f"Return ONLY: x,y (center coordinates) or NOT_FOUND"
        )
        text = agent_llm_call(
            _SCREEN_FIND_SYSTEM, prompt,
            need_vision=True, image_b64=image_b64,
        )
        text = text.strip()
...
def computer_control(
    ...
    try:
        if action == "type":
            ...
            return _scroll(direction=parameters.get("direction", "down"),
            ...
            return _wait_for_image(parameters.get("image", ""),
            ...
            print(f"[ComputerControl] Random {parameters.get('type','name')}: {result}")
            return result
