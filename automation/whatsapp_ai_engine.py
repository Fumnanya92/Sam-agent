...
class WhatsAppAIEngine:
    ...
    def handle_reply_flow(self, player=None):
        ...
        if not message:
            self._speak("I could not detect a message.", player)
            return

        if message["direction"] == "outgoing":
            self._speak("The latest message was sent by you.", player)
            return
        ...
        if not text:
            self._speak("The last message is media content.", player)
            return

        # Read the incoming message aloud
        self._speak(f"{sender} says: {text}", player)

        # Generate AI draft
        draft = generate_reply(text, sender)
        
        if not draft or "error" in draft.lower():
            self._speak("I could not generate a reply.", player)
            return
        ...
        if self._is_sensitive(text) or self._is_sensitive(draft):
            self._speak(
                f"This appears to be a sensitive message. Here is my proposed reply: {draft}. Say 'send it', 'edit', or 'cancel'.",
                player,
            )
        else:
            self._speak(
                f"Here is my proposed reply to {sender}: {draft}. Say 'send it', 'edit', or 'cancel'.",
                player,
            )
    ...
    def confirm_send(self, player=None):
        ...
        if not self.reply_controller.has_pending():
            self._speak("There is no pending reply to send.", player)
            return
        ...
        if success:
            self._speak(f"Reply to {draft_info['receiver']} copied to clipboard. You may paste and send manually.", player)
            self.reply_controller.clear()
        else:
            self._speak("I failed to copy the reply to clipboard.", player)

    def cancel_reply(self, player=None):
        """Handle 'cancel' command - discards the draft."""
        if not self.reply_controller.has_pending():
            self._speak("There is no pending reply to cancel.", player)
            return

        self.reply_controller.clear()
        self._speak("Understood. Draft discarded.", player)

    def edit_reply(self, new_text: str, player=None):
        """Handle 'edit' command - allows user to modify the draft."""
        if not self.reply_controller.has_pending():
            self._speak("There is no pending reply to edit.", player)
            return
        ...
        if new_text and new_text.strip():
            ...
            self._speak(f"Reply updated to: {new_text.strip()}. Say 'send it' or 'cancel'.", player)
        else:
            # Just re-announce the current draft
            self._speak(f"Current draft is: {draft_info['text']}. Please provide new text, or say 'send it' or 'cancel'.", player)

    def get_current_draft(self, player=None):
        """Get info about current pending draft."""
        if not self.reply_controller.has_pending():
            self._speak("There is no pending reply.", player)
            return None

        draft_info = self.reply_controller.get_draft()
        self._speak(f"Pending reply to {draft_info['receiver']}: {draft_info['text']}", player)
        return draft_info
